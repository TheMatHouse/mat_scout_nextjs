...
export async function changeTeamPassword(team, newPassword) {
  if (!team?.security?.wrappedTBK || !team?.security?.kdf) {
    throw new Error("Team security block missing.");
  }

  // 🔐 OPTION A RULE:
  // Password rotation requires an already-unlocked team key
  if (!team._teamKey) {
    throw new Error(
      "Team is locked. Please unlock the team before changing the password."
    );
  }

  // 1️⃣ Use EXISTING session TBK (no unwrap, no old password)
  const tbkRaw = Buffer.from(team._teamKey);

  // 2️⃣ Generate NEW KDF params
  ...
  // 3️⃣ Re-wrap SAME TBK under NEW password
  const newWrapped = wrapTBK(newDerivedKey, tbkRaw);

  // 4️⃣ Persist updated security block
