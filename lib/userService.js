...
// ✅ Get basic user record by Clerk ID
export async function getUserByClerkId(clerkId) {
  return await User.findOne({ clerkId });
}

// ✅ Get full aggregated profile by Clerk ID
export async function getFullUserProfileByClerkId(clerkId) {
  const mongoUser = await getUserByClerkId(clerkId);
  if (!mongoUser) return null;

  const userId = mongoUser._id; // Already an ObjectId, no need to convert
