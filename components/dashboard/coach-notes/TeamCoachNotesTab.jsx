// components/dashboard/coach-notes/TeamCoachNotesTab.jsx
"use client";
...
const TeamCoachNotesTab = ({ user }) => {
  ...
  // --------------------------------------------------------------
  // Load teams user belongs to (owner, manager, coach, member)
  // --------------------------------------------------------------
  useEffect(() => {
    if (!user?._id) return;
  ...
  const goToTeamCoachNotes = (slug) => {
    if (!slug) return;
    router.push(`/teams/${encodeURIComponent(slug)}/coach-notes`);
  };
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      try {
        sessionStorage.setItem(STORAGE_KEY(t._id), pwd);
      } catch {}
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm text-gray-900 dark:text-gray-100">
            Select a team to view Coach Notes.
          </p>
        </div>
...
export default TeamCoachNotesTab;
